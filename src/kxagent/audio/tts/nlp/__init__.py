...
from .symbols import LANGUAGE_ID_MAP, LANGUAGE_TONE_START_MAP, SYMBOLS
...
def extract_bert_feature(
    ...
    if language == Languages.JP:
        from .japanese.bert_feature import extract_bert_feature
    elif language == Languages.EN:
        from .english.bert_feature import extract_bert_feature
    elif language == Languages.ZH:
        from .chinese.bert_feature import extract_bert_feature
    else:
...
def clean_text(
    ...
    if language == Languages.JP:
        from .japanese.g2p import g2p
        from .japanese.normalizer import normalize_text

        norm_text = normalize_text(text)
        phones, tones, word2ph = g2p(norm_text, use_jp_extra, raise_yomi_error)
    elif language == Languages.EN:
        from .english.g2p import g2p
        from .english.normalizer import normalize_text

        norm_text = normalize_text(text)
        phones, tones, word2ph = g2p(norm_text)
    elif language == Languages.ZH:
        from .chinese.g2p import g2p
        from .chinese.normalizer import normalize_text
